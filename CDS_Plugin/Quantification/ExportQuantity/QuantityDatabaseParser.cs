...
using CDS_Plugin.XMLImportExport.Model;
   ...
   public class QuantityDatabaseParser : IDatabaseParser
   {
      ...
      public StepResourceNode ParseStepResource(Int64 rowId)
      {
         ...
         node.Entity = entity;
            ...
            //_______________________________пытаемся отфильтровать primaryquantity из списка, но тут пока собтраем только ID
          
            string output = string.Join("\n", childObjectResource);
            childObjectResource.Clear();

            //MessageBox.Show(output);

            //string a = "primaryquantity_value";
            //long v = long.Parse(a);
            //childObjectResource = childObjectResource.Where(x => x == v).ToList();
            //childObjectResource.RemoveAll(x => x == v);
